...
@freeze_time("2024-12-05")  # Thursday
def test_transform_adds_correct_dates(sample_data):
    ...
    assert result['start_date'].iloc[0] == expected_start
    assert result['end_date'].iloc[0] == expected_end
    assert result['year'].iloc[0] == 2024
    assert result['week_number'].iloc[0] == 49

@freeze_time("2024-12-31")  # Testing year boundary
